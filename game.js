		...
		OOP.drawArr(doxs, function (dox) {
			if (dox.active) {
				if (dox.isStaticIntersect(player)) {
					...
					alert('НАСТЯ, ТЫ УМНИЦА, ИГРА ПРОЧТИ ПРОЙДЕНА, ЖМИ "ОК"');
					alert('ЖМИ ЕЩЁ "ОК"');
					alert('Можешь ещё нажать "ОК"?');
					alert('Над тобой весело издеваться, жми дальше "ОК" хД');
					alert('Не надоело?');
					alert('Впереди тебя ждет кое-что интересное, ты готова?');
					alert('Точно готова?');
					alert('Будем надеяться что готова');
					alert('Прежде, чем наступит "интересное", хочу сказать..');
					alert('Я люблю тебя ^^');
					alert('ВПЕРЁД!!!');				
					location = location.href = "demo/index.html";								
					score++;
				}
			}
		});
